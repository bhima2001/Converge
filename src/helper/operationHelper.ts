...
export const compareKeys = (digitList1: number[], digitList2: number[]): number => {
...
export const generateKey = (leftDigitList: number[], rightDigitList: number[], siteId: number): number[] => {
  const newKey: number[] = []
  let i: number = 0
  while(true) {
    const leftDigit: number = leftDigitList[i] !== undefined ? leftDigitList[i] : 0;
    const rightDigit: number = rightDigitList[i] !== undefined ? rightDigitList[i] : MAX_VALUE;
