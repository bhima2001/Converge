...
export const createRBTree = async(req: requestType, res: responseType) => {
  ...
  res.send({
    status: 200,
    data: "Red Black Tree is successfully created. This is the Inorder Traversal: ", inOrder
  })
}
...
export const insertAt = async (req: requestType, res: responseType) => {
  ...
  const {docId} = req.params
  if(typeof siteId !== "string" || typeof leftDigitList !== "string" || typeof rightDigitList !== "string" || typeof value !== "string"){
  ...
  const key: number[] = generateKey(parsedLeftDigitList, parsedRightDigitList, Number(siteId))
...
export const postOrderDocument = async(req: requestType, res: responseType) => {
  const {docId } =  req.params;
  const serializedTreeData = await client.get(docId)
  console.log("This is serialized Tree data: ",serializedTreeData)
  const rbTree = new RedBlackTree();
...
export const levelOrderTraversal = async (req: requestType, res: responseType) => {
  const {docId } =  req.params;
  const serializedTreeData = await client.get(docId)
  console.log("This is serialized Tree data: ",serializedTreeData)
  const rbTree = new RedBlackTree();
...
export const blackNodesValidation = async (req: requestType, res: responseType) => {
  const {docId} = req.params
  const docInstance: IDocument | null = await Documents.findById(docId).populate('content').exec()
  if(!docInstance){
    res.send({
      status: 404,
...
export const removeCharacter = async(req: requestType, res: responseType) => {
  ...
  const {docId } =  req.params;
  if(typeof value !== 'string' || typeof digitList !== 'string' || typeof siteId !== 'string'){
    res.status(400).send("Invalid query parameter")
    return;
  }
