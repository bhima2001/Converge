...
export const registerUser = async (req: requestType, res: responseType) => {
  ...
  if(usernameCheck){
    res.status(409).send("This Username is taken. Please try with a different username")
    return;
  }
  try{
    const newUser: IUser | null = await User.create({ userName: userName, email: email, password: password });
...
export const login = async (req: requestType, res: responseType) => {
  ...
  if(!user) {
    res.send({
      status: 404,
      message: `The ${userName ? "username" : "email"} and/or password you specified are not correct.`
    })
    return;
  }
